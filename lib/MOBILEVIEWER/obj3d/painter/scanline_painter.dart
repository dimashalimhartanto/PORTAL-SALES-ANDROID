...
import 'package:PortalSales/MOBILEVIEWER/obj3d/painter/scanline_data.dart';
  ...
  if (dP1P2 > dP1P3) {
    for (int y = p1.y.toInt(); y <= p3.y.toInt(); y++) {
      ...
      if (y < p2.y) {
        ...
        processScanLine(canvas, depthBuffer, data, v1, v3, v1, v2, color,
            brightness, texture);
      } else {
        /*
        data.setNdotla(nl1);
        ...
        processScanLine(canvas, depthBuffer, data, v1, v3, v2, v3, color,
            brightness, texture);
      }
    }
  } else {
    ...
    for (int y = p1.y.toInt(); y <= p3.y.toInt(); y++) {
      ...
      if (y < p2.y) {
        ...
        processScanLine(canvas, depthBuffer, data, v1, v2, v1, v3, color,
            brightness, texture);
      } else {
        /*
        data.setNdotla(nl2);
        ...
        processScanLine(canvas, depthBuffer, data, v2, v3, v1, v3, color,
            brightness, texture);
      }
    }
  }
...
void processScanLine(
  ...
  double gradient1 =
      pa.y != pb.y ? ((data.getCurrentY() - pa.y) / (pb.y - pa.y)) : 1;
  double gradient2 =
      pc.y != pd.y ? ((data.getCurrentY() - pc.y) / (pd.y - pc.y)) : 1;
  ...
  for (int x = sx; x < ex; x++) {
    ...
    Color rgb = Color.fromARGB(
        255, (rn * 255).toInt(), (gn * 255).toInt(), (bn * 255).toInt());
    ...
    try {
      ...
      _drawPoint(
          canvas,
          new Math.Vector3(x.toDouble(), data.getCurrentY().toDouble(), z),
          rgb,
          u,
          v);
      //}
