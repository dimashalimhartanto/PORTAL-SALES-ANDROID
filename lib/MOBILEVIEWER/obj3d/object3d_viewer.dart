...
import 'package:PortalSales/MOBILEVIEWER/obj3d/painter/vertical_painter.dart';
import 'package:flutter/material.dart';
import 'package:flutter/services.dart';
import 'package:flutter/widgets.dart';
import 'package:PortalSales/MOBILEVIEWER/obj3d/object3d_model.dart';
import 'package:PortalSales/MOBILEVIEWER/obj3d/painter/globals.dart';
import 'package:PortalSales/MOBILEVIEWER/obj3d/painter/scanline_painter.dart';
import 'package:PortalSales/MOBILEVIEWER/obj3d/painter/texture_data.dart';
import 'package:PortalSales/MOBILEVIEWER/utils/math_utils.dart';
import 'package:PortalSales/MOBILEVIEWER/utils/logger.dart';
import 'package:PortalSales/MOBILEVIEWER/utils/utils.dart';
import 'package:PortalSales/MOBILEVIEWER/widgets/zoom_gesture_detector.dart';
import 'package:PortalSales/MOBILEVIEWER/utils/screen_utils.dart';
...
class Object3DViewerController extends StatefulWidget {
  ...
  useNewAlgorithm(bool useNewAlgorithm) =>
      state.useNewAlgorithm(useNewAlgorithm);
...
class Object3DViewerState extends State<Object3DViewer> {
  ...
  _parse() async {
    ...
    if (!widget.objPath.startsWith("assets/")) {
      ...
      if (await fileObj.exists()) {
        ...
        final contMtl =
            (await fileMtl.exists()) ? await fileMtl.readAsString() : "";
  ...
  _rotationChanged() {
    ...
    if (widget.onRotationChangeListener != null)
  ...
  useNewAlgorithm(bool useNewAlgorithm) {
    widget.rasterizerMethod = useNewAlgorithm
        ? RasterizerMethod.NewMethod
        : RasterizerMethod.OldMethod;
...
class Object3DRenderer extends ChangeNotifier implements CustomPainter {
  ...
  _drawTriangle(
    ...
    Math.Vector3 normalizedLight =
        Math.Vector3.copy(lightPosition).normalized();
    ...
    if (widget.drawMode == DrawMode.WIREFRAME) {
      ...
      if (widget.rasterizerMethod == RasterizerMethod.OldMethod)
        drawTexturedTrianglePoints(
            ...
            color,
            brightness,
            widget.currentState().textureData,
            lightPosition);
  ...
  _clearDepthBuffer() {
    depthBuffer = List.generate(widget.size.width.toInt(),
        (_) => List.filled(widget.size.height.toInt(), double.maxFinite));
  }
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (model.vertices.length > MAX_SUPPORTED_VERTICES) {
      ...
      final sDesc =
          "Too much points: ${model.vertices.length}! Max supported points: $MAX_SUPPORTED_VERTICES";
  ...
  _drawInfo(Canvas canvas, int verticesCount) {
    if (widget.showInfo) {
      ...
      drawText(canvas, "verts: " + verticesCount.toString(),
          Offset(20, ScreenUtils.height - 130),
          fontSize: 14);
      ...
      drawText(canvas, "path: " + widget.objPath,
          Offset(20, ScreenUtils.height - 160),
          fontSize: 12);
    }
  }
  ...
  reset() {
    if (widget.object3DViewerController != null)
      widget.object3DViewerController.reset();
  }
